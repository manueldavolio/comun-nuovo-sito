...
import { isExternalHref } from "@/config/navigation";
    ...
    <section className="bg-[var(--club-page)] pb-6 pt-5 sm:pb-8 sm:pt-6">
      <div className="site-container-home">
        <div className="grid auto-rows-fr grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:gap-4">
          {cards.map((card) => {
            const linkProps = isExternalHref(card.href)
            ...
            return (
              ...
              >
                <Link
                  href={card.href}
                  className="relative block aspect-[16/10] w-full shrink-0 overflow-hidden bg-[#001b3d]"
                  {...linkProps}
                >
                  <Image
                    src={card.image}
